...
local HipHeightSlider = SixthPage.AddSlider("Hip Height(Use Disabler)", {Min = 0, Max = 50, Def = 0}, function(Value)
    HipHeight = Value
    rconsoleprint(HipHeight.." Slider HipHeight")
    rconsoleprint("\n")
end)

local HipHeightButton = SixthPage.AddButton("Set Hip Height(Use Disabler)", function()
...
local ONYXDISABLER = SeventhPage.AddButton("ONYX Disabler", function()
    Player.Character.Humanoid.Name = "LMAOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO"
    rconsolewarn("Disabler Used!")
end)

local ServerLagger = SeventhPage.AddToggle("Server Crasher/Lagger", false, function(Value)
    _G.Lag = Value
    while _G.Lag do
        task.wait()
        game:GetService("ReplicatedStorage").ONYX.Remotes.GunSetup:FireServer(game:GetService("ReplicatedStorage").ONYX.Guns.AK47, true);
    end
end)
